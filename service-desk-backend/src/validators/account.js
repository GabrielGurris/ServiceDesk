...
const rules = {
    ...
    naddress: Joi.number().required(),
    neighborhood: Joi.string().required(),
    cep: Joi.string().pattern(new RegExp('^[0-9]{7,9}$')).required(),
    phone1: Joi.string().pattern(new RegExp('^[0-9]{8,15}$')).required(),
    phone2: Joi.string().pattern(new RegExp('^[0-9]{8,15}$')).required()
}
